...
function findIframeById(pageId) {
    return findTabPanel(pageId).children("div");
}
...
var addTabs = function(options) {
    ...
    if (findTabPanel(pageId) === null) {
        ...
        var $tabPanel = $('<div role="tabpanel" class="tab-pane"></div>').attr(pageIdField, pageId).attr("tab-url", options.url);
        ...
        if (options.content) {
            ...
            //var $iframe = $("<div></div>");
            //iframe 加载完成事件
            // $tabPanel.load($tabPanel.attr('tab-url'),function () {
            // App.unblockUI('#tab-content');//解锁界面
            // App.fixIframeCotent();//修正高度
            // });

            //$tabPanel.append($iframe);
            if ('iframe-tab' === options.targetType) {
                var $iframe = $("<iframe></iframe>").attr("src", options.url).css("width", "100%").attr("frameborder", "no").attr("id", "iframe_" + pageId).addClass("tab_iframe").attr(pageIdField, pageId);
                $tabPanel.append($iframe);
            } else {
                /*$tabPanel.load($tabPanel.attr('tab-url'), function() {
                ...
                $.ajax({
                    url:$tabPanel.attr('tab-url'),
                    async: true,
                    type: 'get',
                    success:function(result){
                        $tabPanel.html(result).addClass('tab_iframe');
                }});
            }
            App.fixIframeCotent();
            $tabPanel.ready(function() {
                // App.init(); 
            });
            /*switch(options.targetType) {
                case 'ajax-tab' : +function(){
                                     $tabPanel.load($tabPanel.attr('tab-url'),function () {
                                    // App.unblockUI('#tab-content');//解锁界面
                                    // App.fixIframeCotent();//修正高度
                                         });
        ...
        //$.parser.parse($tabPanel);  
        // $tab = $(content);

        $("#tab-content").append($tabPanel);
        // $tabPanel.trigger("create");
        // $("#tab-content").trigger("create");
        //iframe 加载完成事件
...
function refreshTabById(pageId) {
    ...
    var $iframe = findTabPanel(pageId);
    var url = $iframe.attr('tab-url');

    $iframe.load($iframe.attr('tab-url'), function() {
        ...
        function init($menu_ul, data, level) {
            $.each(data, function(i, item) {
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    else if (item.targetType != null && item.targetType === "iframe-tab" || item.targetType === "ajax-tab") {
                        item.urlType = item.urlType ? item.urlType : 'relative';
                        var href = 'addTabs({id:\'' + item.id + '\',title: \'' + item.text + '\',close: true,url: \'' + item.url + '\',urlType: \'' + item.urlType + '\',targetType: \'' + item.targetType + '\'});';
                        $a.attr('onclick', href);
